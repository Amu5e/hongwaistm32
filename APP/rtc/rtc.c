...
u8 RTC_Init(void)
{
	...
	{	 			
		...
		RTC_WaitForLastTask();	//等待最近一次对RTC寄存器的写操作完成
		RTC_Set(2023,8,18,17,34,55);  //设置时间	
...
{		 
	...
	{			
		RTC_Get();//更新时间  
		OLED_ShowString(10*8,3*16,":",16);
		OLED_ShowString(13*8,3*16,":",16);
		OLED_ShowNum(7*16,3*16,calendar.sec,2,16);
		OLED_ShowNum(6*16-8,3*16,calendar.min,2,16);
		OLED_ShowNum(5*16-16,3*16,calendar.hour,2,16);
